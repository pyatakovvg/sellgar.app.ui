# Курс Для Преподавателя: `@sellgar/app`
...
Этот раздел превращает техническую документацию `@sellgar/app` в материал для
...
пропускать ментальную модель runtime: именно она отличает `@sellgar/app` от набора
React-компонентов.
...
|---:|---|---|---|
| 1 | [Первый запуск](./01-first-start.md) | Как получить первый экран? | В браузере отображается `Hello, @sellgar/app!` |
...
- Cleanup показывается в том же занятии, где появляется subscription или другой
  долгоживущий ресурс.
- Пример импортирует framework API только из `@sellgar/app`.
